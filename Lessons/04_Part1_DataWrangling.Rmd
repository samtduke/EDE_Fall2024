...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #USEFUL USE THIS
...
NTL.TuesdayLake <- filter (NTL.phys.data, year4 == 1999)

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.TuesdayLake <- filter (NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))
```
...
NTL.phys.data.temp <- arrange(NTL.phys.data, desc(temperature_C))
