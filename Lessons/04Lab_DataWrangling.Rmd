...
library(tidyverse)
library(lubridate)
 #The here package allows for better control of relative paths
...
NTL.phys.data <- read.csv(
  file=("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"), 
...
NTL.phys.data_1999 <- filter(NTL.phys.data, year4 == 1999) 


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data_decade <- filter(
  NTL.phys.data,
...
NTL.phys.data_Ex4 <- NTL.phys.data_Ex3 %>% 
  filter(depth==0) %>% 
...
NTL.phys.data_Longer <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "irradiance_type", values_to = "irradiance")
