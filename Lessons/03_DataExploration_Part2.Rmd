...
USGS.flow.data.complete<-read.csv(
  file=here('Data/Processed/USGS_Site02085_Flow_Processed.csv'),
...
# 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 100)

# 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 200)
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "chartreuse") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50, lty = 5,  color = "blue",) +
...
different plots emphasize different asepcts of the data

How can multiple options for data exploration inform our understanding of our data?

> Answer: 

being able to observe different trends
