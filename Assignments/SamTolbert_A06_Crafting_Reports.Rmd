...
```{r project.setup, message=FALSE, warning=FALSE, echo=TRUE, include=TRUE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
knitr::kable(
summary_PM25, col.names = c('County', '$\\mu g/m^3$'), label="Mean Particulates (2.5mm)", caption = "Mean Particulates (2.5mm)", digits= 2
)

kable(
summary_Ozone, col.names = c('County', '$\\mu g/m^3$'), label="Mean Ozone", caption = "Mean Ozone", digits= 2
)
```
...
\newpage
```{r, fig.height=2.6, echo=FALSE, fig.align='left', fig.cap= 'Ozone 2018'}

summary_Ozone_Month <-epa_data %>%
  mutate(Month = factor(Month, levels = 1:12, labels=month.name))

summary_Ozone_Month_2018<-summary_Ozone_Month %>% 
  drop_na(Ozone) %>%
  filter(Year==2018) %>% 
  group_by(Month)

Box_Ozone_2018<-summary_Ozone_Month_2018 %>%  
  ggplot(aes(x= Month, y= Ozone)) +  
           geom_boxplot()+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=45, hjust=1))+
  labs(y= expression(Ozone ~ (mu ~ g/m^3)))+
  ggtitle('Ozone 2018') +
  theme(plot.title = element_text(hjust=.5),
        plot.margin = margin (5,5,5,5))


print(Box_Ozone_2018)
```

```{r, fig.height=2.6, fig.align='left', echo=FALSE, fig.cap = 'Ozone 2019'}

summary_Ozone_2019<-summary_Ozone_Month %>% 
  drop_na(Ozone) %>%
  filter(Year==2019) %>% 
  group_by(Month)

Box_Ozone_2019<-summary_Ozone_2019 %>%  
  ggplot(aes(x= Month, y= Ozone)) +  
           geom_boxplot()+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=45, hjust=1))+
...
<https://github.com/samtduke/EDE_Fall2024>
