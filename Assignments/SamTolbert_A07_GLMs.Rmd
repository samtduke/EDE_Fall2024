...
NTLitter<-read.csv(here( "Data", "Raw",
                         "NTL-LTER_Lake_ChemistryPhysics_Raw.csv"),
                  stringsAsFactors = T
                  )

NTLitter$sampledate<-mdy(NTLitter$sampledate)
...
NTLitterScatter<-ggplot(NTLitterWrangled,
                        aes(x=depth, y=temperature_C, color=temperature_C))+
                        geom_point(alpha=.6)+
                        scale_color_gradient(low= 'blue', high= 'red')+
                          ylim(0, 35)+
                            geom_smooth(method="lm", color='green')+
                            labs(color= "Temperature", x= 'Depth', y= 'Temperature (Celsius)', title= 'Temperature by Depth')+
  theme(plot.title=element_text(hjust=.5, color='navy'),
        legend.position = 'none')

print(NTLitterScatter)
